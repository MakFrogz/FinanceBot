...
def get_users_keyboard(user_id):
    ...
    for _id in users_id:
        if not _id == user_id:
            btn = telebot.types.InlineKeyboardButton(text=user.get_user_fullname(_id), callback_data='u_' + str(_id))
            markup.row(btn)
    return markup
...
def get_new_markup(old_markup, callback):
    ...
    for i in new_markup['inline_keyboard']:
        if i[0]['callback_data'] == callback:
            ...
            break
    if not [{'text': 'Подтвердить', 'callback_data': 'apply'}, {'text': 'Отменить', 'callback_data': 'cancel'}] in new_markup['inline_keyboard']:
